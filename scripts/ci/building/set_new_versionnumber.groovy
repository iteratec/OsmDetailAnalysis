...
if ( !bamboo_jira_version && !bamboo_jira_version_manually ) {
    println 'None master-version given from jira...'
    newVersion = "${oldVersion}"
}
if (bamboo_jira_version) {
    println 'Given master-version from jira: ' + bamboo_jira_version + '...'
    newVersion = bamboo_jira_version
}
if (bamboo_jira_version_manually) {
    println 'Given master-version from manual run: ' + bamboo_jira_version_manually + '...'
...
if (!System.getenv("bamboo_planRepository_branchName").equals("master")) {
    newVersion += "-SNAPSHOT"
}
