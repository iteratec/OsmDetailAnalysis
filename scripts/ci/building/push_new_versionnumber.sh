...
if [ -z $bamboo_jira_version ] && [ -z $bamboo_jira_version_manually ]; then
    ...
    if [ "${bamboo_planRepository_branchName}" = "master" ]; then
      echo "Found jira version-number in master branch: ${bamboo_jira_version}"
      ...
      remote_url=https://$bamboo_git_USER_NAME:$bamboo_git_PASSWORD@github.com/iteratec/OsmDetailAnalysis.git
      ...
      echo "pull --rebase $remote master"
      git pull --rebase $remote master >> ./push_new_versionnumber_out

      echo "git tag 'v${jira_version}'"
      git tag "v${jira_version}" >> ./push_new_versionnumber_out
      echo "git push --tags $remote HEAD:refs/heads/master"
      git push --tags $remote HEAD:refs/heads/master >> ./push_new_versionnumber_out

      echo "push_new_versionnumber_out:"
      cat ./push_new_versionnumber_out
    else
      echo 'Wrong branch. Committing only into the master branch.'
    fi
fi